...
const register = catchAsync(async (req, res, next) => {
  ...
  // كل حساب عميل self-signup لازم يترافق معاه Client document مرتبط
  // (user: user._id)، وإلا مش هيقدر يعمل أي حاجة محتاجة "client id"
  // زي إنشاء استشارة، لأن مفيش سجل عميل مرتبط بحسابه أصلاً.
  await Client.create({
    user: user._id,
