...
const createUser = catchAsync(async (req, res, next) => {
  const { firstName, lastName, email, password, role, phone, branch } = req.body;
  ...
  const user = await User.create({ firstName, lastName, email, password, role, phone, branch });
...
const updateUser = catchAsync(async (req, res, next) => {
  const allowedFields = ['firstName', 'lastName', 'phone', 'role', 'branch', 'isActive'];
  const updates = {};
